    ...
    def login(self, request):
        ...
        if serializer.is_valid():
            ...
            if user:
                ...
                return Response({'token': token.key, 'firstname':  user.first_name, 'lastname': user.last_name, 'email': user.email, 'isAdmin': user.is_superuser, 'detail': 'Login successful'}, status=status.HTTP_200_OK)
            else:
